...
?>
    ...
    unset($_SESSION['registration_message']);
      ...
      ?>
                    ...
                    <form class="form-sample" id="AdminRegistration" action="process/register-Counselor.php" method="POST">
...
?>
  ...
  // Function to generate and check the counselorID
  function generatecounselorID() {
    ...
    // Function to increment and check if the counselorID exists in the database
    function checkcounselorIDExists(counselorID) {
        const xhr = new XMLHttpRequest();
        xhr.open("POST", "validation/counselor-id-validation.php", true);
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                if (xhr.responseText === "exists") {
                    counselorID++;  // Increment the ID
                    checkcounselorIDExists(counselorID);  // Check the new ID
                } else {
                    document.getElementById("counselorID").value = padID(counselorID); // Set the ID
                }
            }
        };
        xhr.send("counselorID=" + padID(counselorID));  // Send padded ID to the server for validation
    }

    checkcounselorIDExists(counselorID); // Start with the initial ID and check if it exists
  }
  ...
  function checkEmail() {
    ...
    const xhr = new XMLHttpRequest();
    xhr.open("POST", "validation/counselor-check-email.php", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  ...
  function checkUsername() {
    ...
    const xhr = new XMLHttpRequest();
    xhr.open("POST", "validation/counselor-check-username.php", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
