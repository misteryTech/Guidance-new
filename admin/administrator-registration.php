...
?>
    ...
    unset($_SESSION['registration_message']);
      ...
      ?>
                    ...
                    <form class="form-sample" id="AdminRegistration" action="process/register-administrator.php" method="POST">
...
?>
  ...
  // Function to generate and check the adminID
  function generateAdminID() {
    ...
    // Function to increment and check if the adminID exists in the database
    function checkAdminIDExists(adminID) {
        const xhr = new XMLHttpRequest();
        xhr.open("POST", "validation/admin-id-validation.php", true);
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                if (xhr.responseText === "exists") {
                    adminID++;  // Increment the ID
                    checkAdminIDExists(adminID);  // Check the new ID
                } else {
                    document.getElementById("adminID").value = padID(adminID); // Set the ID
                }
            }
        };
        xhr.send("adminID=" + padID(adminID));  // Send padded ID to the server for validation
    }

    checkAdminIDExists(adminID); // Start with the initial ID and check if it exists
  }
  ...
  function checkEmail() {
    ...
    const xhr = new XMLHttpRequest();
    xhr.open("POST", "validation/admin-check-email.php", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  ...
  function checkUsername() {
    ...
    const xhr = new XMLHttpRequest();
    xhr.open("POST", "validation/admin-check-username.php", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
